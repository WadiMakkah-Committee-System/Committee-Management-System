 ...
 * (min_length=1 حسب CommitteeFormationRequestCreate بالباك-إند).
 */
export function MemberPicker({ users, selected, onChange, error }: MemberPickerProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        className={cn(
          'max-h-56 overflow-y-auto rounded-sm border',
      ...
      >
        {filtered.length === 0 ? (
          ...
          filtered.map((u) => (
            <label
              key={u.user_id}
              className="flex cursor-pointer items-center gap-2.5 border-b border-border-default px-3 py-2 text-sm transition-colors last:border-0 hover:bg-bg-elevated"
            >
              <input
                type="checkbox"
