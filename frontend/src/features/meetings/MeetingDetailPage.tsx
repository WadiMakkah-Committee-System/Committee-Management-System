...
export function MeetingDetailPage() {
  ...
  // إصلاح أداء 2026-09-15 (تحليل.pdf — Frontend Finding #2، المرحلة
  // الرابعة): كان الاستعلامان يُطلَقان بلا شرط عند فتح الصفحة (enabled
  // فقط بشرط !!meetingId) بينما قسم "التسجيل والمسودة" نفسه (أدناه) لا
  // يُعرَض إطلاقًا إلا لو meetingEnded — يعني كل فتحة لاجتماع لم ينتهِ بعد
  // (الحالة الأكثر شيوعًا: اجتماعات قادمة/جارية) كانت تُطلق طلبين HTTP
  // زائدين مضمونين بلا فائدة (لا نتيجة تُعرَض لهما إطلاقًا). نفس نمط
  // الحراسة المستخدَم أصلًا بـextractedItemsQuery أدناه (meetingEnded ?
  // meetingId : undefined) — بمجرد meetingEnded=true يُفعَّل الاستعلامان
  // تلقائيًا (React Query تعيد التفعيل فور تغيّر enabled)، فلا أثر على أي
  // سلوك حالي: كل استخدامات recordingQuery/draftQuery بالملف (تأثير
  // التوليد التلقائي + قسم العرض + شرط "البنود المستخرجة" أسفل الصفحة)
  // أصلًا محروسة بـmeetingEnded بشكل مباشر أو غير مباشر.
  const recordingQuery = useMeetingRecording(meetingEnded ? meetingId : undefined)
  const draftQuery = useMeetingDraft(meetingEnded ? meetingId : undefined)
  const generateDraftMutation = useGenerateMeetingDraft()
  const downloadRecordingMutation = useDownloadMeetingRecording()
  const playRecordingMutation = usePlayMeetingRecording()
  // تشغيل التسجيل بالصفحة (طلب لاما 2026-09-13: "زر تشغيل بدل تنزيل") —
  // الرابط يُبنى مرة واحدة فقط لأول ضغطة تشغيل (لا إعادة جلب عند
  // pause/play اللاحقة)، ويُحرَّر (revokeObjectURL) عند تفكيك الصفحة
  // لتفادي تسريب الذاكرة.
  ...
  async function handleTogglePlayRecording() {
    if (!meetingId) return
    ...
    try {
      const url = await playRecordingMutation.mutateAsync(meetingId)
  ...
  useEffect(() => {
    ...
    // إصلاح 2026-09-13 (بلاغ لاما — "كل ما ادخل اجتماع يرجع يسوي المسودة
    // مرة ثانية"): قبل هذا السطر، كان الشرط يعتمد على draftQuery.data
    // فقط — لو recordingQuery.data يوصل (يتحمّل) قبل draftQuery.data
    // (سباق تحميل عادي جدًا)، draft تكون لسه undefined (مو null) لحظة
    // تنفيذ الشرط، فـ"draft && (...)" يفشل رغم وجود مسودة completed
    // فعليًا بالقاعدة — والتوليد التلقائي يُطلَق من جديد فوق مسودة
    // موجودة أصلًا. الحل: لا تقرّري شيء قبل ما تنتهي draftQuery فعليًا
    // من أول تحميل لها.
    if (draftQuery.isLoading || !draftQuery.isFetched) return
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [meetingEnded, meetingId, recordingQuery.data, draftQuery.data, draftQuery.isLoading, draftQuery.isFetched])
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <Button
                        size="sm"
                        variant="secondary"
                        icon={recordingAudioUrl ? <Pause size={14} /> : <Play size={14} />}
                        onClick={handleTogglePlayRecording}
                        loading={playRecordingMutation.isPending}
                      >
                        {recordingAudioUrl ? 'إيقاف' : 'تشغيل'}
                      </Button>
                      <Button
                        size="sm"
                        variant="secondary"
