...
engine = create_async_engine(
    ...
    # بوضع Session.
    pool_pre_ping=True,
    # تحديث 2026-09-15 (تحقيق أداء لاما، المرحلة السادسة — بيانات فعلية من
    # لوحة Supabase: Database → Connection Pooling): "Connection pool size"
    # هناك = 15 اتصال فعلي فقط بـPostgres نفسه (حسب حجم الـCompute الحالي،
    # Nano)، وهذا الحد مُصرَّح عنه صراحة بنفس الصفحة إنه "Shared across all
    # connection poolers" — يعني يُقسَم بين منفذ Session (5432) ومنفذ
    # Transaction (6543) المستخدَم هنا معًا، وليسا حصتين منفصلتين. كان
    # pool_size=10 + max_overflow=10 هنا يسمح للباك-إند وحده بطلب لغاية 20
    # اتصال محلي — سقف وهمي أصلًا (Supavisor ما يملك غير 15 حقيقية يوزّعها
    # بغض النظر عن الإعداد المحلي)، وأخطر من كونه وهميًا: يترك صفر هامش لأي
    # استخدام آخر لنفس الـ15 بنفس اللحظة (لوحة Supabase Studio، أدوات فحص
    # القاعدة، Migrations يدوية) — لو تصادف تزامن عالٍ من الباك-إند مع أي
    # استخدام آخر كهذا، النتيجة تضارب/رفض اتصالات فعلي بدل انتظار منظّم
    # ومتوقَّع بطابور SQLAlchemy المحلي نفسه (queue داخل الـPool، مقيسة
    # أصلًا بحدث pool_checkout أعلاه بالتطوير).
    #
    # التخفيض إلى 8+4 (حد أقصى 12) يترك هامش أمان صريح (3 اتصالات) دائمًا
    # فاضية لأي استخدام آخر، بتكلفة نظرية شبه معدومة: الفرق (12 مقابل 15)
    # ما ينطبق إلا بأسوأ لحظة تزامن ممكنة بالباك-إند *و* بأفضل ظرف ممكن (لا
    # أحد آخر يستخدم القاعدة بنفس اللحظة) — سيناريو نادر، خصوصًا إن الباك-
    # إند نفسه محدود بـ0.15 CPU فقط بخطة Render الحالية (سقف حسابي أضيق
    # بكثير من أي سقف اتصالات). لو زاد التزامن الحقيقي مستقبلًا لدرجة يصير
    # فيها الانتظار ملموسًا، الحل الصحيح رفع "Connection pool size" نفسه من
    # لوحة Supabase (يتطلب ترقية حجم الـCompute — قرار تكلفة تخص لاما، وليس
    # تعديل كود).
    pool_size=8,
    max_overflow=4,
    pool_recycle=300,
